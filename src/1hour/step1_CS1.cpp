...
int main(int argc, char *argv[]){
  ...
  string s4(argv[4]); //Hour no.
  map<string, vector<double> > mapTimeData = read_data(s1);
  cout<<"Number of time slot is "<<mapTimeData.size()<<endl;
  auto endread=chrono::high_resolution_clock::now();
  chrono::duration<double> diffread = endread-startread;
  cout << "One hour read data Runtime is: " << diffread.count() << "s" << endl;
  ...
  for(auto iter = mapTimeData.begin(); iter != mapTimeData.end(); ++iter){
    ...
    for(auto iter2=x.begin(); iter2!=x.end(); ++iter2){
      ...
      // chrono::duration<double> diffsum = endsum-startsum;
      // cout << "One hour sum usage Runtime is: " << diffsum.count() << "s" << endl;
     }
  ...
  pt_ratio.close();
   ...
   auto starthour=chrono::high_resolution_clock::now();
  ...
  // for(int64_t i=0 ; i<24 ; i++){
    ...
    result_HM.close();
    auto endhour=chrono::high_resolution_clock::now();
    chrono::duration<double> diffhour = endhour-starthour;
    cout << "One hour LUT Runtime is: " << diffhour.count() << "s" << endl;
  // }//endfor
