...
int main(int argc, char *argv[]){
  ...
  vector<vector<int64_t> > dec_result1(sum_row_count_AM);
  vector<vector<int64_t> > dec_result2(div_row_count_HM);
  vector<Plaintext> poly_dec_result1, poly_dec_result2;
  for(int i1=0 ; i1<sum_row_count_AM ; i1++){
    Plaintext ex;
    Ciphertext exx;
    poly_dec_result1.push_back(ex);
    ct_result1.push_back(exx);
  }
  for(int i2=0 ; i2<div_row_count_HM ; i2++){
    Plaintext ex;
    Ciphertext exx;
    poly_dec_result2.push_back(ex);
    ct_result2.push_back(exx);
  }
  ...
  ifstream result_1;
  result_1.open(s2+"/inv_SUM_AM_"+s1, ios::binary);
  Ciphertext temp1;
  for(int w1 = 0; w1 < sum_row_count_AM ; w1++) {
  ...
  ifstream result_2;
  result_2.open(s2+"/div_HM_"+s1, ios::binary);
  Ciphertext temp2;
  for(int w2 = 0; w2 < div_row_count_HM ; w2++) {
  ...
  for (int z1=0 ; z1<sum_row_count_AM ; z1++){
    //cout<<"i: "<<z<<endl;
  ...
  omp_set_num_threads(NF);
  #pragma omp parallel for
  for (int z2=0 ; z2<div_row_count_HM ; z2++){
    //cout<<"i: "<<z<<endl;
  ...
  cout<<"index_row_y:"<<index_row_y<<", index_col_y:"<<index_col_y<<endl;
  ...
  ofstream queryFile;
  queryFile.open(s2+"/pir_SUM_AM_DIV_HM_"+s1);
